...
get_cosmic_target_data <- function(targeted_path){
  valid_mut_type <- c("coding_sequence_variant", "coding_sequence_variant,splice_acceptor_variant",
                      "coding_sequence_variant,splice_donor_variant", "inframe_deletion",
                      "inframe_deletion,start_lost", "inframe_deletion,stop_gained",
                      "inframe_deletion,stop_lost", "inframe_insertion",
                      "inframe_insertion,splice_acceptor_variant", "inframe_insertion,stop_gained",
                      "missense_variant", "splice_acceptor_variant", "splice_donor_variant",
                      "start_lost", "stop_gained", "stop_lost", "stop_retained_variant", "")
  
  target_data <- arrow::open_dataset(targeted_path, format = "tsv") %>%
    dplyr::select(c("GENE_SYMBOL", "COSMIC_SAMPLE_ID", "MUTATION_DESCRIPTION", "COSMIC_PHENOTYPE_ID")) %>%
...
merge_counts <- function(target_counts, genome_counts){
  ...
  gene_counts <- merge(target_counts, genome_counts, by = c("GENE_SYMBOL", "SITE_PRIMARY"), all = TRUE)
