...
combinations <- expand.grid(year = years, gene = genes, stringsAsFactors = FALSE) %>%
...
get_citation_count <- function(term) {
  tryCatch({
    ...
    search_result <- entrez_search(db="pubmed", term=term, use_history = TRUE)$count
...
results <- data.frame(search_term = character(), count = integer(), stringsAsFactors = FALSE)
...
pending_combinations <- combinations[!combinations$search_term %in% processed_terms,]
...
for (i in 1:nrow(pending_combinations)) {
  ...
  # Append new result
  new_result <- data.frame(search_term = term, count = count, stringsAsFactors = FALSE)
